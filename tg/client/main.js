...
// В development: localhost:3000
// В production: ваш отдельный backend домен
import { initTelegramAuth, getAuthHeaders, getStoredUser } from '../../client/telegramAuth.js';

const API_BASE =
  window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1'
    ? 'http://localhost:3000'
    : (window.GAME_CONFIG?.API_BASE || window.location.origin);
...
const IS_TELEGRAM = typeof window !== 'undefined' && window.Telegram && window.Telegram.WebApp;
...
function setFrameHighlight(frameEl, state) {
  if (!frameEl) return;
  frameEl.classList.remove('success', 'error', 'load-error');
  if (state) {
...
function beginImageLoad(imgEl, frameEl, url, onLoadCallback, imageCache = mpState.preloadedImages) {
  ...
  if (cachedImg && cachedImg.complete && cachedImg.naturalWidth > 0 && cachedImg.naturalHeight > 0) {
    ...
    if (cachedImg.naturalWidth >= 10 && cachedImg.naturalHeight >= 10) {
      ...
      imgEl.src = url;
      frameEl.classList.remove('loading', 'error', 'load-error');
  ...
  const timeout = setTimeout(() => {
    if (imgEl.dataset.loadingToken === token) {
      ...
      frameEl.classList.remove('loading');
      frameEl.classList.add('load-error');
      delete imgEl.dataset.loadingToken;
      if (onLoadCallback) {
        onLoadCallback();
      }
...
function showLevelUpAnimation(level) {
  ...
  let animationContainer = document.getElementById('level-up-animation');
  ...
  animationContainer.classList.remove('active', 'fade-out');
  
  // Создаем основной элемент с текстом
  const levelUpText = document.createElement('div');
  levelUpText.className = 'level-up-text';
  levelUpText.innerHTML = `
    <div class="level-up-title">🎉 УРОВЕНЬ ПОВЫШЕН! 🎉</div>
    <div class="level-up-number">Уровень ${level}</div>
  ...
  for (let i = 0; i < 20; i++) {
    ...
    const angle = (360 / 20) * i * (Math.PI / 180); // Преобразуем в радианы
  ...
  requestAnimationFrame(() => {
    ...
    // После завершения основной анимации добавляем fade-out
    setTimeout(() => {
      animationContainer.classList.add('fade-out');
      
      // Удаляем анимацию после полного исчезновения
      setTimeout(() => {
        animationContainer.classList.remove('active', 'fade-out');
        animationContainer.innerHTML = '';
      }, 500);
  ...
  // Воспроизводим звуковой эффект (если доступен Web Audio API)
  try {
...
const singleHintBtn = document.getElementById('single-hint-btn');
const singleHintsCountEl = document.getElementById('single-hints-count');
const singleSkipBtn = document.getElementById('single-skip-btn');
const singleSkipsCountEl = document.getElementById('single-skips-count');
const singleBuyHintsBtn = document.getElementById('single-buy-hints-btn');
const singleStreakSaveRow = document.getElementById('single-streak-save-row');
const singleStreakSaveBtn = document.getElementById('single-streak-save-btn');
const singleStreakSavesCountEl = document.getElementById('single-streak-saves-count');
...
async function fetchCredits() {
  ...
  try {
    const [h, s, ss] = await Promise.all([
      fetch(`${API_BASE}/api/hints`, { headers }).then((r) => r.ok ? r.json() : {}),
      fetch(`${API_BASE}/api/skips`, { headers }).then((r) => r.ok ? r.json() : {}),
      fetch(`${API_BASE}/api/streak-saves`, { headers }).then((r) => r.ok ? r.json() : {}),
    ]);
    singleState.hintsCount = h.unlimited ? 999 : (h.count ?? 0);
    singleState.hintsUnlimited = !!h.unlimited;
    singleState.skipsCount = s.count ?? 0;
    singleState.streakSavesCount = ss.count ?? 0;
    updateCreditsUI();
  } catch {}
}
const fetchHintsCount = fetchCredits;

function updateCreditsUI() {
  const auth = getAuthHeaders();
  const hasAuth = !!Object.keys(auth).length;
  const h = singleState.hintsCount ?? 0;
  const sk = singleState.skipsCount ?? 0;
  const ss = singleState.streakSavesCount ?? 0;
  if (singleHintBtn) {
    singleHintBtn.classList.toggle('hidden', !hasAuth || h < 1 || !singleState.question || singleState.hintUsedThisQuestion);
    if (singleHintsCountEl) singleHintsCountEl.textContent = singleState.hintsUnlimited ? '∞' : h;
  }
  if (singleSkipBtn) {
    singleSkipBtn.classList.toggle('hidden', !hasAuth || sk < 1 || !singleState.question);
    if (singleSkipsCountEl) singleSkipsCountEl.textContent = sk;
  }
  if (singleBuyHintsBtn) singleBuyHintsBtn.classList.toggle('hidden', !hasAuth || (h > 0 && !singleState.hintsUnlimited));
  if (singleStreakSaveRow) singleStreakSaveRow.classList.toggle('hidden', !singleState.streakSaveOffered);
  if (singleStreakSavesCountEl) singleStreakSavesCountEl.textContent = ss;
  const floatBtn = document.getElementById('float-premium-btn');
  if (floatBtn) floatBtn.classList.toggle('hidden', !hasAuth || h > 0 || singleState.hintsUnlimited || sk > 0 || ss > 0);
}
const updateHintsUI = updateCreditsUI;
...
const profileManager = (() => {
  ...
  const platform = {
    ...
    async init() {
      ...
      try {
        ...
        // Получаем CloudStorage API (если доступен)
        if (this.webApp.CloudStorage) {
          this.cloudStorage = this.webApp.CloudStorage;
        }
        
        // Загружаем данные из CloudStorage или используем данные пользователя Telegram
        const user = this.webApp.initDataUnsafe?.user;
        if (user) {
          // Используем имя пользователя Telegram как начальный alias
          const telegramAlias = user.first_name || user.username || 'Игрок';
          state.alias = telegramAlias;
        }
        ...
        console.warn('[profile] Telegram WebApp init failed', error);
        return false;
      }
    },
    async loadFromStorage() {
      if (!this.cloudStorage) {
        ...
        const saved = localStorage.getItem('game_profile');
        if (saved) {
          try {
            const data = JSON.parse(saved);
            applyLoadedData(data);
          } catch (e) {
            console.warn('[profile] Failed to parse saved data', e);
          }
        }
        return;
      }
      ...
      try {
        const keys = ['alias', 'totalXp', 'stats', 'achievements'];
        const data = {};
        ...
        console.warn('[profile] Failed to load from CloudStorage', error);
        // Fallback на localStorage
        const saved = localStorage.getItem('game_profile');
        if (saved) {
          try {
    ...
    async save(payload) {
      if (!this.cloudStorage) {
        // Fallback на localStorage
        try {
          const current = JSON.parse(localStorage.getItem('game_profile') || '{}');
          const updated = { ...current, ...payload };
          localStorage.setItem('game_profile', JSON.stringify(updated));
          return true;
        } catch (error) {
          console.warn('[profile] Failed to save to localStorage', error);
          return false;
        }
      }
      ...
      try {
        for (const [key, value] of Object.entries(payload)) {
          await this.cloudStorage.setItem(key, JSON.stringify(value));
        }
        return true;
      } catch (error) {
        console.warn('[profile] Failed to save to CloudStorage', error);
        // Fallback на localStorage
        try {
          const current = JSON.parse(localStorage.getItem('game_profile') || '{}');
          const updated = { ...current, ...payload };
  ...
  async function addXp(amount) {
    ...
    // Сохраняем старый уровень для отслеживания повышения
...
async function loadSingleQuestion(nextQuestion) {
  try {
    ...
    singleState.hintUsedThisQuestion = false;
    singleState.streakSaveOffered = false;
    if (singleImageEl && singleFrameEl) {
      // Очищаем все классы состояния перед загрузкой нового изображения
      singleFrameEl.classList.remove('success', 'error', 'load-error');
    ...
    // Показываем сообщение об ошибке только если это не ошибка загрузки изображения
    // (ошибки загрузки изображения обрабатываются в beginImageLoad)
    if (singleFeedbackEl && error.message && !error.message.includes('Failed to load') && !error.message.includes('timeout')) {
...
async function answerSingle(event) {
  ...
  try {
    ...
    if (!resp.ok) {
      // Если вопрос не найден (404), просто загружаем новый вопрос
      if (resp.status === 404) {
        singleState.isBusy = false;
        // Очищаем текущий вопрос и загружаем новый
        singleState.question = null;
        if (singleFeedbackEl) {
          singleFeedbackEl.textContent = '';
          singleFeedbackEl.classList.remove('error', 'success');
        }
        loadSingleQuestion().catch(() => {});
        return;
      }
    ...
    if (data.correct) {
      ...
      singleState.questionsAnswered = (singleState.questionsAnswered || 0) + 1;
      if (singleState.score >= 10 && (singleState.hintsCount ?? 0) === 0 && (singleState.skipsCount ?? 0) === 0 && (singleState.streakSavesCount ?? 0) === 0 && IS_TELEGRAM) {
        setTimeout(() => {
          showToast('🛡 Защити серию! Профиль → Premium');
          document.getElementById('profile-toggle')?.click();
        }, 600);
      } else if (singleState.questionsAnswered > 0 && singleState.questionsAnswered % 5 === 0 && (singleState.hintsCount ?? 0) === 0 && IS_TELEGRAM) {
        setTimeout(() => showToast('💡 Подсказки защитят серию — Профиль → Premium'), 600);
      }
    } else {
      const scoreBeforeWrong = singleState.score;
      singleState.scoreBeforeWrong = scoreBeforeWrong;
      singleState.questionsAnswered = (singleState.questionsAnswered || 0) + 1;
      singleState.score = 0;
      singleFeedbackEl.textContent =
      ...
      if (IS_TELEGRAM && window.Telegram?.WebApp?.openInvoice) {
        if (scoreBeforeWrong >= 5 && (singleState.streakSavesCount ?? 0) === 0) {
          setTimeout(() => {
            showToast(`Потеряно ${scoreBeforeWrong} очков. Страховка за 3 ⭐ спасла бы.`);
            document.getElementById('profile-toggle')?.click();
            setTimeout(() => document.getElementById('profile-about')?.click(), 200);
          }, 600);
        } else {
          setTimeout(() => showToast('💡 Подсказки спасут серию — купи в профиле'), 500);
        }
      }
      if ((singleState.streakSavesCount ?? 0) > 0) {
        singleState.streakSaveOffered = true;
        ...
        singleState.nextQuestionTimer = window.setTimeout(() => {
          singleState.streakSaveOffered = false;
          singleState.question = null;
          if (singleFeedbackEl) singleFeedbackEl.textContent = '';
          if (singleStreakSaveRow) singleStreakSaveRow.classList.add('hidden');
          updateCreditsUI();
    ...
    singleBestEl.textContent = singleState.best;
    const nextQ = data.nextQuestion || (singleState.preloadedQuestions.length > 0 ? singleState.preloadedQuestions.shift() : null);
    if (!singleState.streakSaveOffered) {
      if (nextQ) {
        singleState.nextQuestionTimer = window.setTimeout(() => {
          if (singleFeedbackEl) {
            singleFeedbackEl.textContent = '';
    ...
    console.error(error);
    // Показываем ошибку только если это не ошибка загрузки изображения
    if (error.message && !error.message.includes('Failed to load') && !error.message.includes('timeout')) {
      ...
      // Через 2 секунды очищаем ошибку и загружаем новый вопрос
      setTimeout(() => {
        if (singleFeedbackEl) {
          singleFeedbackEl.textContent = '';
          singleFeedbackEl.classList.remove('error');
...
async function useHint() {
  if (singleState.isBusy || !singleState.question || singleState.hintUsedThisQuestion || (singleState.hintsCount ?? 0) < 1) return;
  singleState.isBusy = true;
  try {
    const r = await fetch(`${API_BASE}/api/hints/use`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...getAuthHeaders() },
      body: JSON.stringify({ questionId: singleState.question.questionId }),
    });
    const data = await r.json();
    if (!r.ok) {
      if (data.error === 'no_hints') showToast('Нет подсказок');
      return;
    }
    singleState.hintsCount = data.count ?? 0;
    singleState.hintUsedThisQuestion = true;
    const correctAnswer = data.correctAnswer;
    updateHintsUI();
    if (singleState.hintsCount === 0) setTimeout(() => showToast('Подсказки закончились — купи ещё'), 1500);
    if (correctAnswer && singleFeedbackEl) {
      singleFeedbackEl.textContent = correctAnswer === 'real' ? 'Это реальное фото.' : 'Это нейросеть.';
      singleFeedbackEl.classList.remove('error');
      singleFeedbackEl.classList.add('success');
      setFrameHighlight(singleFrameEl, 'success');
      const correctBtn = document.querySelector(`#single-player .choice-button[data-answer="${correctAnswer}"]`);
      if (correctBtn) correctBtn.classList.add('hint-revealed');
      singleState.nextQuestionTimer = window.setTimeout(() => {
        singleState.question = null;
        singleFeedbackEl.textContent = '';
        singleFeedbackEl.classList.remove('success');
        if (correctBtn) correctBtn.classList.remove('hint-revealed');
        loadSingleQuestion();
        singleState.isBusy = false;
        singleState.nextQuestionTimer = null;
      }, 2000);
...
singleHintBtn?.addEventListener('click', useHint);
singleBuyHintsBtn?.addEventListener('click', () => openPremiumInvoice(1));

async function useSkip() {
  if (singleState.isBusy || !singleState.question || (singleState.skipsCount ?? 0) < 1) return;
  singleState.isBusy = true;
  try {
    const r = await fetch(`${API_BASE}/api/skips/use`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...getAuthHeaders() },
      body: JSON.stringify({}),
    });
    if (!r.ok) return;
    const data = await r.json();
    singleState.skipsCount = data.count ?? 0;
    singleState.question = null;
    if (singleFeedbackEl) singleFeedbackEl.textContent = '';
    updateCreditsUI();
    loadSingleQuestion();
...
singleSkipBtn?.addEventListener('click', useSkip);

singleStreakSaveBtn?.addEventListener('click', async () => {
  if (singleState.isBusy || !singleState.streakSaveOffered || (singleState.streakSavesCount ?? 0) < 1) return;
  singleState.isBusy = true;
  if (singleState.nextQuestionTimer) {
    window.clearTimeout(singleState.nextQuestionTimer);
    singleState.nextQuestionTimer = null;
  ...
  try {
    const r = await fetch(`${API_BASE}/api/streak-saves/use`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...getAuthHeaders() },
      body: JSON.stringify({}),
    });
    if (!r.ok) return;
    const data = await r.json();
    singleState.streakSavesCount = data.count ?? 0;
    singleState.score = singleState.scoreBeforeWrong ?? singleState.score;
    singleState.streakSaveOffered = false;
    singleScoreEl.textContent = singleState.score;
    if (singleFeedbackEl) singleFeedbackEl.textContent = 'Серия сохранена!';
    singleFeedbackEl?.classList.remove('error');
    singleFeedbackEl?.classList.add('success');
    singleState.question = null;
    if (singleStreakSaveRow) singleStreakSaveRow.classList.add('hidden');
...
document.getElementById('single-share-btn')?.addEventListener('click', async () => {
  if (!cachedBotUsername) {
    try {
      const r = await fetch(`${API_BASE}/auth/telegram/config`);
      if (r.ok) {
        const cfg = await r.json();
        cachedBotUsername = cfg.botUsername || 'Arboo34_bot';
      }
    } catch {}
    if (!cachedBotUsername) cachedBotUsername = 'Arboo34_bot';
  }
  const score = singleState.score || singleState.best || 0;
  const user = getStoredUser();
  const ref = user?.id ? `?start=ref_${user.id}` : '';
...
function initializeApp() {
  Promise.all([
    initTelegramAuth(API_BASE),
    profileManager.init(),
  ]).then(([auth]) => {
    if (auth?.enabled && auth?.user) {
      const displayName = auth.user.displayName || [auth.user.first_name, auth.user.last_name].filter(Boolean).join(' ') || auth.user.username || `User${auth.user.id}`;
      profileManager.setAlias(displayName);
      const statusEl = document.getElementById('profile-auth-status');
      if (statusEl) {
        statusEl.textContent = `Вошли как ${displayName}`;
        statusEl.classList.remove('hidden');
      }
      const widgetContainer = document.getElementById('profile-telegram-auth');
      if (widgetContainer) {
        const hint = widgetContainer.querySelector('.profile-auth-hint');
  ...
  socket.addEventListener('message', (event) => {
    ...
    switch (type) {
      ...
      case 'question':
        ...
        // Очищаем все классы состояния перед загрузкой нового изображения
        if (mpFrameEl) {
          mpFrameEl.classList.remove('success', 'error', 'load-error');
        }
...
async function renderLeaderboard() {
  ...
  // TODO: Интеграция с лидербордом (можно реализовать через backend API)
  // Лидерборд можно реализовать через backend API
  // или использовать Telegram-специфичные функции
}
...
function openAboutModal() {
  ...
  fetch(`${API_BASE}/auth/telegram/config`).then((r) => r.ok ? r.json() : {}).then((c) => {
    const link = document.getElementById('honeygain-link');
    if (link && c.honeygainRef) {
      link.href = c.honeygainRef.startsWith('http') ? c.honeygainRef : `https://r.honeygain.com/${c.honeygainRef}`;
    } else if (link) {
      link.href = 'https://www.honeygain.com/';
    }
  }).catch(() => {});
}
...
// Premium: купить
async function openPremiumInvoice(productOrPack, extra) {
  if (!IS_TELEGRAM || !window.Telegram?.WebApp?.openInvoice) {
    showToast('Оплата Stars доступна только в Telegram');
    return;
  }
  const btns = document.querySelectorAll('#buy-hints-1, #buy-hints-3, #buy-hints-5, #buy-hints-10, #buy-hints-20, #buy-skip, #buy-streak, #buy-streak-3, #buy-unlimited, #buy-tip, #single-buy-hints-btn');
  btns.forEach((b) => { if (b) b.disabled = true; });
  let reqBody;
  if (productOrPack === 'streak_save' && extra) {
    reqBody = { product: 'streak_save', count: extra };
  } else if (productOrPack === 'unlimited_hints') {
    reqBody = { product: 'unlimited_hints' };
  } else if (productOrPack === 'tip') {
    reqBody = { product: 'tip' };
  } else if (typeof productOrPack === 'number') {
    reqBody = { pack: productOrPack };
  } else {
    reqBody = { product: productOrPack };
  }
  try {
    const resp = await fetch(`${API_BASE}/api/invoice/create`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(reqBody),
    });
    const data = await resp.json();
    if (!data.url) throw new Error(data.error || 'Нет ссылки');
    window.Telegram.WebApp.openInvoice(data.url, (status) => {
      btns.forEach((b) => { b.disabled = false; });
      if (status === 'paid') {
        const msg = reqBody.product === 'skip' ? '+1 пропуск!' : reqBody.product === 'streak_save' ? `+${reqBody.count || 1} страховок!` : reqBody.product === 'unlimited_hints' ? 'Безлимит 24ч!' : reqBody.product === 'tip' ? 'Спасибо!' : `+${data.count || reqBody.pack || 5} подсказок!`;
        showToast(msg);
        fetchCredits().catch(() => {});
        if (window._rematchAfterPay) {
          window._rematchAfterPay = false;
          closeMatchStatsModal();
          document.getElementById('multiplayer-tab')?.click();
          setTimeout(() => document.getElementById('mp-queue-btn')?.click(), 300);
        }
      }
    });
  } catch (err) {
    btns.forEach((b) => { b.disabled = false; });
    showToast('Ошибка: ' + (err.message || 'не удалось создать счёт'));
  }
}
document.getElementById('buy-hints-1')?.addEventListener('click', () => openPremiumInvoice(1));
document.getElementById('buy-hints-3')?.addEventListener('click', () => openPremiumInvoice(3));
document.getElementById('buy-hints-5')?.addEventListener('click', () => openPremiumInvoice(5));
document.getElementById('buy-hints-10')?.addEventListener('click', () => openPremiumInvoice(10));
document.getElementById('buy-hints-20')?.addEventListener('click', () => openPremiumInvoice(20));
document.getElementById('buy-skip')?.addEventListener('click', () => openPremiumInvoice('skip'));
document.getElementById('buy-streak')?.addEventListener('click', () => openPremiumInvoice('streak_save'));
document.getElementById('buy-streak-3')?.addEventListener('click', () => openPremiumInvoice('streak_save', 3));
document.getElementById('buy-unlimited')?.addEventListener('click', () => openPremiumInvoice('unlimited_hints'));
document.getElementById('buy-tip')?.addEventListener('click', () => openPremiumInvoice('tip'));
document.getElementById('footer-premium-link')?.addEventListener('click', (e) => {
  e.preventDefault();
  document.getElementById('profile-toggle')?.click();
  setTimeout(() => document.getElementById('profile-about')?.click(), 100);
});
document.getElementById('float-premium-btn')?.addEventListener('click', (e) => {
  e.preventDefault();
  document.getElementById('profile-toggle')?.click();
  setTimeout(() => document.getElementById('profile-about')?.click(), 100);
});
...
function showMatchStats({ headline, didWin, myStats, oppStats, opponentAlias, durationMs, totalQuestions, xpGained }) {
  ...
  matchStatsContent.innerHTML = `
    ...
    <div class="match-stats-actions">
      <button id="match-rematch-btn" class="primary-button">⏭ Реванш + 1 пропуск (2 ⭐)</button>
    </div>
  `;
  
  matchStatsBackdrop.classList.remove('hidden');
  matchStatsModal.classList.remove('hidden');
  document.getElementById('match-rematch-btn')?.addEventListener('click', () => {
    if (!IS_TELEGRAM || !window.Telegram?.WebApp?.openInvoice) {
      showToast('Оплата только в Telegram');
      return;
    }
    openPremiumInvoice('skip');
    window._rematchAfterPay = true;
  });
}
