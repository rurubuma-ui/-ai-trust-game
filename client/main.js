...
const API_BASE =
  window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1'
    ? 'http://localhost:3000'
    : (window.GAME_CONFIG?.API_BASE || 'https://api.your-domain.com'); // ← Замените на ваш домен!

const WS_BASE = API_BASE.replace(/^http/, 'ws');

// Проверка локального окружения (не в iframe Яндекс Игр)
const IS_LOCAL_DEV = 
  window.location.hostname === 'localhost' || 
  window.location.hostname === '127.0.0.1' ||
  window.parent === window; // Не в iframe

  // Подавление ошибок SDK при локальной разработке
  if (IS_LOCAL_DEV && typeof window !== 'undefined') {
    const originalConsoleError = console.error;
    const originalConsoleWarn = console.warn;
    
    // Фильтруем ошибки SDK Яндекс Игр
    console.error = function(...args) {
      const message = args.join(' ');
      // Пропускаем только ошибки SDK Яндекс Игр, связанные с локальным запуском
      if (message.includes('No parent to post message') ||
          message.includes('Can not get appId from environment') ||
          message.includes('SDK environment') ||
          message.includes('YandexGamesSDKEnvironment') ||
          message.includes('localStorage is broken on iOS/MacOS')) {
        // Тихо игнорируем эти ошибки в локальной разработке
        return;
      }
      originalConsoleError.apply(console, args);
    };
    
    // Также фильтруем предупреждения
    console.warn = function(...args) {
      const message = args.join(' ');
      if (message.includes('YaGames init failed') && message.includes('No parent to post message')) {
        // Тихо игнорируем это предупреждение
        return;
      }
      originalConsoleWarn.apply(console, args);
    };
    
    // Обработка необработанных Promise rejections от SDK
    // Устанавливаем обработчик с максимальным приоритетом (capture phase)
    window.addEventListener('unhandledrejection', (event) => {
      ...
      // Подавляем все ошибки SDK Яндекс Игр при локальной разработке
      if (errorString.includes('No parent to post message') ||
          errorString.includes('Can not get appId from environment') ||
          errorString.includes('SDK environment') ||
          errorString.includes('postToParent') ||
          errorString.includes('YandexGamesSDKEnvironment') ||
          errorString.includes('localStorage is broken')) {
    ...
    // Также перехватываем через window.onunhandledrejection
...
const profileManager = (() => {
  ...
  const platform = {
    ...
    player: null,
    async init() {
      // В локальной разработке пропускаем инициализацию YaGames
      if (IS_LOCAL_DEV) {
        return false;
      }
      ...
      if (typeof window !== 'undefined' && typeof window.YaGames === 'function') {
        try {
          ...
          // В локальной разработке не показываем это предупреждение
          if (!IS_LOCAL_DEV) {
  ...
  try {
    ...
    if (nextQ) {
      singleState.nextQuestionTimer = window.setTimeout(() => {
        // Очищаем сообщение об ошибке перед загрузкой следующего изображения
        if (singleFeedbackEl) {
          singleFeedbackEl.textContent = '';
          singleFeedbackEl.classList.remove('error', 'success');
        }
...
function initializeApp() {
  Promise.all([
    ...
    preloadSingleQuestions(5).then(() => loadSingleQuestion()).catch(() => loadSingleQuestion());
    updateAchievementsSummary();
  }).catch((error) => {
