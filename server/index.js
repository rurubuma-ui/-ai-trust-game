...
import { requireMoltbookAgent } from './moltbook.js';
import { verifyInitData, verifyLoginWidget } from './telegramAuth.js';
...
function updateManifest(manifest) {
  ...
  // Обновляем кэш локальных AI-изображений (local + Moltbook agent submissions)
  localAiImages = imageManifest.filter(item =>
    item.id && (item.id.startsWith('local-ai-') || item.id.startsWith('molt-agent-')) && item.type === 'ai'
  );
...
const app = express();
// CORS настройка: разрешаем запросы от Яндекс Игр и localhost
...
// --- Telegram auth: login via bot ---
const TELEGRAM_BOT_TOKEN = process.env.TELEGRAM_BOT_TOKEN;
const telegramSessions = new Map(); // token -> { user, expires }
...
app.post('/auth/telegram', (req, res) => {
  if (!TELEGRAM_BOT_TOKEN) {
    return res.status(503).json({ error: 'telegram_auth_disabled', hint: 'TELEGRAM_BOT_TOKEN not set' });
  }
  const { initData, ...loginWidgetData } = req.body || {};
  ...
  if (!user) {
    return res.status(401).json({ error: 'invalid_telegram_auth', hint: 'Send initData (Web App) or Login Widget hash' });
  }
  ...
  console.log(`[telegram] User logged in: ${displayName} (${user.id})`);
  res.json({
    success: true,
    token,
...
app.get('/auth/telegram/config', (_req, res) => {
  ...
  const frontendUrl = process.env.FRONTEND_URL || process.env.API_BASE?.replace(/\/api.*$/, '') || '';
  res.json({
    enabled: !!TELEGRAM_BOT_TOKEN,
    botUsername,
    authCallbackUrl: `${process.env.API_BASE || `http://localhost:${PORT}`}/auth/telegram/callback`,
    frontendUrl,
  });
});

// Login Widget redirect: Telegram sends user here with GET params
app.get('/auth/telegram/callback', (req, res) => {
  ...
  if (!user) {
    ...
    return res.redirect(`${frontend}?auth_error=invalid`);
  }
  const { token } = createTelegramSession(user);
  const frontend = process.env.FRONTEND_URL || '/';
  res.redirect(`${frontend}?auth_token=${encodeURIComponent(token)}`);
});

// --- Telegram Stars: create invoice link for donations ---
app.post('/api/invoice/create', async (req, res) => {
  if (!TELEGRAM_BOT_TOKEN) {
    return res.status(503).json({ error: 'payments_disabled' });
  }
  const amount = Math.min(Math.max(parseInt(req.body?.amount, 10) || 10, 5), 1000);
  try {
    const resp = await fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/createInvoiceLink`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        title: 'Поддержка AI Trust Experiment',
        description: 'Добровольная поддержка проекта. Stars выводятся в TON.',
        payload: JSON.stringify({ type: 'donation', ts: Date.now() }),
        provider_token: '',
        currency: 'XTR',
        prices: [{ label: 'Поддержка', amount }],
      }),
    });
    ...
    res.json({ url: data.result, amount });
  } catch (err) {
...
// --- Moltbook: AI agent image submission ---
const AGENT_IMAGES_DIR = path.join(IMAGES_DIR, 'ai');
const AGENT_IMAGE_MAX_SIZE = 5 * 1024 * 1024; // 5 MB
const storage = multer.diskStorage({
  destination: (_req, _file, cb) => {
    fs.mkdirSync(AGENT_IMAGES_DIR, { recursive: true });
    cb(null, AGENT_IMAGES_DIR);
  },
  filename: (_req, file, cb) => {
    const ext = path.extname(file.originalname) || '.jpg';
    const safeExt = ['.jpg', '.jpeg', '.png', '.webp'].includes(ext.toLowerCase()) ? ext : '.jpg';
    cb(null, `agent-${Date.now()}-${uuid().slice(0, 8)}${safeExt}`);
  },
});
const uploadAgentImage = multer({
  storage,
  limits: { fileSize: AGENT_IMAGE_MAX_SIZE },
  fileFilter: (_req, file, cb) => {
    const allowed = /\.(jpe?g|png|webp)$/i.test(file.originalname);
    cb(null, allowed);
  },
});

app.post('/api/images/submit', requireMoltbookAgent, uploadAgentImage.single('image'), (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({ error: 'image_required', hint: 'Send multipart/form-data with field "image"' });
    }
    const agent = req.moltbookAgent;
    const prompt = (req.body?.prompt || '').toString().slice(0, 500);
    const apiBase = process.env.API_BASE || `http://localhost:${PORT}`;
    const imageId = `molt-agent-${agent.id.slice(0, 8)}-${uuid().slice(0, 8)}`;
    const imageUrl = `${apiBase.replace(/\/$/, '')}/images/ai/${req.file.filename}`;
    ...
    const entry = {
      ...
      attribution: {
        author: agent.name,
        profile: `https://www.moltbook.com/agent/${agent.id}`,
        sourceUrl: 'https://www.moltbook.com',
      },
    };
    ...
    console.log(`[moltbook] Agent ${agent.name} (${agent.id}) submitted image ${imageId}`);
    res.status(201).json({
      success: true,
      imageId,
      url: imageUrl,
      message: 'Image added to the game. It will appear in rounds soon!',
    });
  } catch (err) {
    console.error('[moltbook] Submit error:', err);
    res.status(500).json({ error: 'submit_failed' });
  }
});

app.get('/api/moltbook/auth-info', (_req, res) => {
  const apiBase = process.env.API_BASE || `http://localhost:${PORT}`;
  res.json({
    authUrl: 'https://moltbook.com/auth.md',
    submitEndpoint: `${apiBase}/api/images/submit`,
    instructions: 'Get identity token from Moltbook, then POST image with X-Moltbook-Identity header',
  });
});
